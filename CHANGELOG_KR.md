...
### 거버넌스
- **Agora 투표 가중치가 위임 반영 방식으로 전환**: 각 투표의 가중치는 제안의 스냅샷 블록 시점 **위임된 MOC 투표권**(`ERC20Votes` — `getPastVotes`)으로 산정되며, 한 번도 위임하지 않은 보유자는 **잔액 fallback**(`balanceOf`)이 적용됩니다. 1 MOC = 1 vote는 그대로 적용됩니다 — 가중치는 위임된 MOC 투표권으로 산정되며, 한 번도 위임하지 않았다면 잔액으로 산정됩니다.

### 문서
- README(EN/KR)와 Governance v1.0 문서(EN/KR)를 Agora의 **위임된 MOC 투표권** 가중(잔액 fallback 포함) 방식으로 업데이트(기존에는 원 MOC 잔액 가중으로 기술)
- Passport의 두 참여 메커니즘을 분리하여 기존의 뭉뚱그린 서술을 정정: **시그널 투표**는 Agora 투표를 대체하지 않는 비구속적 여론 신호이며, Passport의 **위임(delegation)** 은 이제 공식 MOC 가중 Agora 투표에 **반영**됩니다

### 참고사항
- 거버넌스 모델(제안 프로세스, 투표 규칙, 멤버십)은 **변경 없음** — 이번 릴리스는 Agora의 위임 반영 투표 가중을 반영하고 관련 Passport 문구를 정교화
- 투표자 안내: MOC로 투표하려면 별도 조치가 필요 없습니다 — 잔액이 그대로 집계됩니다(잔액 fallback); 위임하면 위임받은 사람이 당신의 가중치로 투표하며, 자기 자신에게 위임하면 본인 투표권을 유지합니다
